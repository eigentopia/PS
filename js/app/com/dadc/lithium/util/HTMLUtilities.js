...
HTMLUtilities.removeHTMLTags = function(text) {
	...
	try{
	    ...
	    Logger.log("strInputCode= "+strInputCode);
	    var strTagStrippedText = strInputCode.replace(/<\/?[^>]+(>|$)/g, "");
	    strTagStrippedText = strTagStrippedText.replace(/\u000a|\u000d/g, '');
	    Logger.log("strTagStrippedText= "+strTagStrippedText);
...
HTMLUtilities.deEnt = function( s )
{
	s = s.replace( /&([^;]+);/g, HTMLUtilities.doReplace );
	return s;
}

HTMLUtilities.doReplace = function( str, p1 )
{
	if ( p1[ 0 ] == "#" )
	{
		p1 = p1.substr( 1 );
		...
		if ( HTMLUtilities.commonASCII[ p1 ] ) return HTMLUtilities.commonASCII[ p1 ];
		else return String.fromCharCode( p1 );
	}
	else
	{
		return HTMLUtilities.commonEntities[ p1 ];
	}
}

HTMLUtilities.commonASCII =
{
