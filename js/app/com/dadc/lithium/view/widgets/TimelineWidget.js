...
var TimelineWidget = function( current_time, total_video_length, ad_breaks ) {
    var This = this;
    ...
    var m_navigation_control_widget = new NavigationControlWidget(
        [
            {control: NavigationControlWidget.CONTROLS.CIRCLE, caption: Dictionary.getText( Dictionary.TEXT.BACK ), x_offset: 300},
            {control: NavigationControlWidget.CONTROLS.START, caption: Dictionary.getText( Dictionary.TEXT.PAUSE ), x_offset: 500},
            {control: NavigationControlWidget.CONTROLS.DPAD_RIGHT_LEFT, caption: Dictionary.getText( Dictionary.TEXT.SEEK ), x_offset: 700},
            {control: NavigationControlWidget.CONTROLS.TRIANGLE, caption: Dictionary.getText( Dictionary.TEXT.SETTINGS ), x_offset: 900},
        ] );
    ...
    this.refreshWidget = function( current_time, total_video_length, ad_breaks ){
        ...
        Logger.log( 'timeline refresh' );
        Logger.log( 'ad breaks' );
        //Logger.logObj( ad_breaks );
    ...
    this.showCursor = function(){
        ...
        if ( ! m_time_container.contains( m_cursor_time_indicator_container ) ){
            ...
            m_cursor_time_indicator_container.addChild( m_cursor_time_text_container );
        }
//        m_time_container.addChild( m_current_time_text_container );
    ...
    this.hideCursor = function(){
        if ( m_time_container.contains( m_cursor_time_indicator_container ) ){
            m_cursor_time_indicator_container.removeChild( m_cursor_time_text_container );
    ...
    this.scanForward = function(){
        ...
        if( m_seek_time + 60 >= m_total_video_length ){
    ...
    function addAdBreaks(){
        for( var i in m_ad_breaks ){
            var time_pos = m_ad_breaks[ i ];
    ...
    function updateCursorText(){
        ...
        if( m_cursor_time_indicator_container.contains( m_cursor_time_text_container ) ){
            m_cursor_time_indicator_container.removeChild( m_cursor_time_text_container );
            visible = true;
        }
        ...
        m_cursor_time_text_container.x = 17;
        m_cursor_time_text_container.y = 10;     
    ...
    m_navigation_control_widget.getDisplayNode().x = 180;
