...
var ShowsMenuController = function( ParentControllerObj ){
    ...
    this.getControllerName = function(){return 'ShowsMenuController';};
    ...
    this.update = function( engine_timer ){
        if( LoggerConfig.CONFIG.UPDATE_DEBUG ) Logger.log( 'ShowsMenuController update() ' + engine_timer );
    ...
    this.prepareToOpen = function( ){
        ...
        var genreFiltersRequest = new LocaleGenreRequest( 'shows', StorageManagerInstance.get( 'geocode' ), function( LocaleGenreObj, status ){
            if ( status != 200 ){
...
//        ParentControllerObj.notifyPreparationStatus( m_unique_id, Controller.PREPARATION_STATUS.STATUS_READY );
    ...
    function featured_CALLBACK(){
        m_parent_controller_obj.requestingParentAction(
            {action: ApplicationController.OPERATIONS.OPEN_FEATURED, calling_controller: this, category: FeaturedRequest.CATEGORY.SHOWS }
        );        
    }

    function popular_CALLBACK(){
        m_parent_controller_obj.requestingParentAction(
            {action: ApplicationController.OPERATIONS.OPEN_POPULAR, calling_controller: this, category: PopularRequest.CATEGORY.SHOWS }
        );
    }

    function recent_CALLBACK(){
        m_parent_controller_obj.requestingParentAction(
            {action: ApplicationController.OPERATIONS.OPEN_RECENT, calling_controller: this, category: RecentRequest.CATEGORY.SHOWS }
        );        
    }

    function browse_CALLBACK(){
        m_parent_controller_obj.requestingParentAction(
            {action: ApplicationController.OPERATIONS.OPEN_BROWSEALL, calling_controller: this, category: BrowseRequest.CHANNEL_NAME.SHOWS }
        );        
    }
    
    function genre_CALLBACK( genre ){
        m_parent_controller_obj.requestingParentAction(
            {action: ApplicationController.OPERATIONS.OPEN_GENRE, genre: genre, calling_controller: this, category: BrowseRequest.CHANNEL_NAME.SHOWS }
        );        
    }
    /**
