...
var WatchlistMenuController = function( ParentControllerObj ){
    ...
    this.getControllerName = function(){return 'WatchlistMenuController';};
    ...
    this.update = function( engine_timer ){
        if( LoggerConfig.CONFIG.UPDATE_DEBUG ) Logger.log( 'WatchlistMenuController update() ' + engine_timer );
        
        m_sub_menu_widget.update( engine_timer );
    }
    ...
    this.prepareToOpen = function( ){
        m_menu_items = [];    
        var browseRequest = new BrowseRequest( BrowseRequest.CHANNEL_NAME.COLLECTIONS, BrowseRequest.FILTER_TYPE.ALL, 'All', BrowseRequest.SORT_ORDER.DATE, StorageManagerInstance.get( 'geocode' ), function( BrowseObj, status ){
            if ( status != 200 ){
                ...
                for( var i = 0; i < BrowseObj.getItemList().getTotalItems(); i++ ){
                    var browse_item = BrowseObj.getItemList().getItem( i );

                    // Create a menu item with the collection name and collection id 
                    // as an argument to the callback function
                    m_menu_items.push( new MenuItem( browse_item.getName(), null, collection_CALLBACK, browse_item.getID() ) );
                }

                m_menu_items.unshift(new MenuItem (Dictionary.getText( Dictionary.TEXT.MY_WATCHLIST), null, mywatchlist_callback, 00))
                // Refresh the menu widget with MenuItemObjs
                m_sub_menu_widget.refreshWidget( m_menu_items, "" );

                //add My Watchlist to top
    ...
    function collection_CALLBACK( collection_id ){
        m_parent_controller_obj.requestingParentAction(
            {action: ApplicationController.OPERATIONS.OPEN_COLLECTION, calling_controller: this, category: BrowseRequest.CHANNEL_NAME.COLLECTIONS, collection_id: collection_id }
        );        
    }
    function mywatchlist_callback( ){
        var user = ApplicationController.getUserInfo();

        if(user.id !=null){
            m_parent_controller_obj.requestingParentAction(
                {action: ApplicationController.OPERATIONS.OPEN_MY_WATCHLIST, calling_controller: this}
            ); 

        }
        else{
            m_parent_controller_obj.requestingParentAction(
                {action: ApplicationController.OPERATIONS.OPEN_LOGIN, calling_controller: this, previousAction:{action: ApplicationController.OPERATIONS.OPEN_MY_WATCHLIST, calling_controller: this}}
            );
        }
    }
    /**
