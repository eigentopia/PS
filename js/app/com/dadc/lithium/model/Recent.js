...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
var RecentRequest = function( RecentRequest_CATEGORY, RecentRequest_FILTER_TYPE, geo_code, num_results, callback ){
    //SAMPLE URL
    //http://api.crackle.com/Service.svc/recent/movies/all/us/30?format=json
    var url = ModelConfig.getServerURLRoot() + "recent/" + RecentRequest_CATEGORY + "/" + RecentRequest_FILTER_TYPE + "/" + geo_code + "/" + num_results + "?format=json";
    Logger.log( url );
    var httpRequestObj;
    var api_retries;
    var http_retries;
    ...
    this.onRequestComplete = function( data, status ){
        if ( status != 200 && http_retries < ModelConfig.CONFIG.NETWORK_ERROR_RETRY ){
            ...
            try{
                var json_data = new Recent( JSON.parse( data ) );
...
var RecentTests = function( ){

    this.runTests = function(){
    // CREATE THE REQUEST WITH SOME ARGS
    var recentTests = new RecentRequest( RecentRequest.CATEGORY.MOVIES, RecentRequest.FILTER_TYPE.ALL, 'us', 30, function( RecentObj ){
    // DO THE ASSERTIONS
        assertEquals( RecentObj.getItemList().getTotalItems(), 30 );

    });
    recentTests.startRequest();
   }
}
...
var Recent = function( json_data ){
    this.m_data = json_data;

    //CREATING OBJECTS
    var statusObj     = new RecentStatus( this.m_data.status );
    var itemListObj   = new RecentItemList( this.m_data.Items );
...
var RecentStatus = function( data ){
...
var RecentItemList = function( data ){
        ...
        for( var i = 0;i < data.length; i++ ){
            itemArray[ i ] = new RecentItem( data[ i ] );
        }
