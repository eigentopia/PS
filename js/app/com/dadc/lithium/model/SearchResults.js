...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
var SearchResultsRequest = function( SearchResultsRequest_ITEM_TYPE, keyword, geocode, callback ){
    //SAMPLE URL
    //http://api.crackle.com/Service.svc/search/all/Ghost/us?format=json
    var url = ModelConfig.getServerURLRoot() + "search/" + SearchResultsRequest_ITEM_TYPE + "/" + escape( keyword ) + "/" + geocode + "?format=json";
    Logger.log( url );
    var httpRequestObj;
    var api_retries;
    var http_retries;
    ...
    this.onRequestComplete = function( data, status ){
        if ( status != 200 && http_retries < ModelConfig.CONFIG.NETWORK_ERROR_RETRY ){
            ...
            try{
                var json_data = new SearchResults( JSON.parse( data ) );
...
var SearchResultsTests = function( ){

    this.runTests = function(){
    // CREATE THE REQUEST WITH SOME ARGS
    var searchResultsTests = new SearchResultsRequest( SearchResultsRequest.ITEM_TYPE.ALL, 'Ghost', 'us', function( searchObj ){
    // DO THE ASSERTIONS
        assertEquals( searchObj.getItemList().getItem(0).getItemType(), "Channel");

    });
    searchResultsTests.startRequest();
   }
}
...
var SearchResults = function( json_data ){
    this.m_data = json_data;

    //CREATING OBJECTS
    var statusObj     = new SearchResultsStatus( this.m_data.status );
    var itemListObj   = new SearchResultsList( this.m_data.Items );
...
var SearchResultsStatus = function( data ){
...
var SearchResultsList = function( data ){
    ...
    for( var i = 0;i < data.length; i++ ){
        itemArray[ i ] = new SearchResultsItem( data[ i ] );
    }
}

var SearchResultsItem = function( data ){
...
SearchResultsRequest.ITEM_TYPE = {
    CHANNEL: "channel",
    MEDIA: "media",
    ALL: "all"
};